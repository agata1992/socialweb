...
class GroupsController extends Controller
{
	public function indexAction(CookieService $cookie_service,DBService $db_service,
		...
		if((ceil($count_groups/$groups_on_page)<($page) && $page != 1) || $page <= 0)
			...
			return $this->render('groups.html.twig', array(
				...
				'groups' => $groups,'page' => $page,'category' => $category,
				'search_input' => $name)
			);
	}
	
	public function create_groupAction(CookieService $cookie_service,
	...
	public function show_groupAction(CookieService $cookie_service,
		DBService $db_service,$id,$subpage,$page
	){
		$user = $cookie_service->check_exist_user_cookie();
		...
		for($i =0;$i<count($group_users);$i++){
			$group_users[$i]['age'] = date_diff(new \DateTime(),
				$group_users[$i]['birthdate']
			)->y;
		}
		...
		if($subpage == "edycja"){
			if($group_owner->getid() !=  $user_data[0])
				return $this->render('page_no_found.html.twig',array(
					'page_name' => 'Nie znaleziono strony','nav_title' => 'Grupa',
					'user' => $user_data)
				);
			else
				return $this->render('group_settings_edit.html.twig', array(
					'page_name' => 'Grupa','nav_title' => 'Edytuj grupę',
					'user' => $user_data,'group' => $group,'group_owner' => $group_owner,
					'group_users' => $group_users,'subpage' => $subpage,'page' => $page)
				);
		}
		elseif($subpage == "dyskusja"){ 
			$group_comments = $db_service->getgroup_comments($id);
			$group_comments_array = array();
			
			for($i = 0;$i<count($group_comments);$i++)
				$group_comments_array[$i] = $group_comments[$i]['id'];
				
			$group_subcomments = $db_service->getgroup_subcomments($group_comments_array);
			
			$comments_on_page = 6;
			$count_comments = count($group_comments);
			
			if((ceil($count_comments/$comments_on_page) <($page) && $page != 1) || $page <= 0)
				return $this->render('page_no_found.html.twig',array(
					'page_name' => 'Nie znaleziono strony','nav_title' => 'Grupa',
					'user' => $user_data)
				);
			...
			return $this->render('group_page.html.twig', array(
				'page_name' => 'Grupa','nav_title' => 'Grupa','user' => $user_data,
				'group' => $group,'group_owner' => $group_owner,
				'group_users' => $group_users,'comments' => $group_comments,
				'subcomments' => $group_subcomments,'subpage' => $subpage,'page' => $page)
			);
		}
		else
			return $this->render('group_page.html.twig', array(
	...
	public function join_unjoin_groupAction(CookieService $cookie_service,
		DBService $db_service,Request $request,$id,$subpage,$page
	){
		if($request->isXmlHttpRequest() == "true"){
			$user = $cookie_service->check_exist_user_cookie();
