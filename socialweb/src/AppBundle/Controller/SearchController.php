	...
	public function indexAction(CookieService $cookie_service,DBService $db_service,
		...
		if($name != ''){
			...
			for($i = 0;$i < $count_users;$i++){
				$searched_users[$i]->age = date_diff(new \DateTime(),
					$searched_users[$i]->getbirthdate()
				)->y;
				$friendship = $db_service->check_friend($searched_users[$i]);
				$searched_users[$i]->friendship = $friendship;
				
				if(!($searched_users[$i]->age >= $tmp_min_age && 
					$searched_users[$i]->age <= $tmp_max_age) || 
					$searched_users[$i]->getid() == $user_data[0]
				){
		...
		return $this->render('search.html.twig', array(
			'page_name' => 'Wyszukiwarka','nav_title' => 'Szukaj','user' => $user_data,
			'searched_users' => $searched_users,'searcher_user_name' => $name,
			'searcher_user_city' => $city,'searcher_user_gender' => $gender,
			'searcher_user_min_age' => $min_age,'searcher_user_max_age' => $max_age,
			'page' => $page)
		);
	}
}