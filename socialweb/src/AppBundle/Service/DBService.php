...
use AppBundle\Entity\Group_Comments;
use AppBundle\Entity\Group_Subcomments;
...
class DBService
{
	...
	public function get_user_data()
	{
		...
		$this->user_data_array = array($this->user_data->getid(),
			$this->user_data->getname(),$this->user_data->getsurname(),
			$this->user_data->getemail(),$this->user_data->getpassword(),
			$this->user_data->getsalt(),$this->user_data->getcity(),
			$this->user_data->getbirthdate(),$this->user_data->getgender(),
			$this->user_data->getprofile_img()
		);
	...
	public function get_user_album_by_id($album_id,$user_profile_data ='')
	{
		if($user_profile_data == ''){
			...
			->getRepository(Albums::class)->findOneBy([
				'user_id' => $this->user_data_array[0],'id'=>$album_id
			]);
		}
		else{
			$album = $this->entityManager
			->getRepository(Albums::class)->findOneBy([
				'user_id' => $user_profile_data->getid(),'id'=>$album_id
			]);
			
			if(($album->getalbum_access() == 1) || ($album->getalbum_access() == 2 &&
				$this->check_friend($user_profile_data) != 1)
			)
				$album = null;
		}
		return $album;
	...
	public function check_album_exists($album_name)
	{
		...
		$album = $this->entityManager
		->getRepository(Albums::class)->findOneBy([
			'user_id' => $this->user_data_array[0],'album_name'=>$album_name
		]);
	...
	public function add_user($name,$surname,$email,$password,$salt,$city,
		$birthdate,$gender
	){
		$user = new Users();
		$user->setname($name);
	...
	public function search_users($name,$city,$gender)
	{
		...
		->where('CONCAT(u.name,  \' \',u.surname) LIKE :name and 
			u.id <> :my_id and u.city LIKE :city'
		)->setParameter(":name", '%'.$name.'%')
		->setParameter(":city", '%'.$city.'%')
		->setParameter(":my_id",$this->user_data_array[0]);
		
		if($gender != '')
			$search_users = $search_users->andwhere('u.gender = :gender')
	...
	public function check_friend($searched_user)
	{
		...
		->getRepository(Friends::class)->findOneBy([
			'user1_id' => $this->user_data_array[0],'user2_id' => $searched_user->getid()
		]);
		
		$friends2 = $this->entityManager
		->getRepository(Friends::class)->findOneBy([
			'user1_id' => $searched_user->getid(),'user2_id' => $this->user_data_array[0]
		]);
		
		$invitations = $this->entityManager
		->getRepository(Invitations::class)->findOneBy([
			'user1_id' => $this->user_data_array[0],'user2_id' => $searched_user->getid()
		]);
	...
	public function invitations($friend_id)
	{
		...
		if(!$user == null){
			...
			->getRepository(Friends::class)->findOneBy([
				'user1_id' => $this->user_data_array[0],'user2_id' =>$friend_id
			]);
			
			$friends2 = $this->entityManager
			->getRepository(Friends::class)->findOneBy([
				'user1_id' => $friend_id,'user2_id' => $this->user_data_array[0]
			]);
			...
			if($friends1 == null && $friends2 == null && $invitations1 == null &&
				$invitations2 == null
			){
	...
	public function get_user_profile_data($user_id)
	{
		...
		{
			$user_profile_data_array = array($user_profile_data->getid(),
				$user_profile_data->getname(),$user_profile_data->getsurname(),
				$user_profile_data->getemail(),$user_profile_data->getpassword(),
				$user_profile_data->getsalt(),$user_profile_data->getcity(),
				$user_profile_data->getbirthdate(),$user_profile_data->getgender(),
				$user_profile_data->getprofile_img()
			);
	...
	public function get_friends($user_id,$search_input)
	{
		...
		->where('f.user1_id = :user or  f.user2_id = :user')
		...
		$friends = $this->entityManager
		->getRepository(Users::class)->createQueryBuilder('u')
		->orderBy('CONCAT(u.name,  \' \',u.surname)', 'ASC')
		->where('u.id in  (:friends_id_array)')
		->setParameter(":friends_id_array",$friends_id_array);
	...
	public function getimage_comments($image_id)
	{
		...
		->createQueryBuilder()
		->select('ii.id,ii.user_id,ii.add_date,ii.text','u.name,u.surname,u.profile_img')
		->from('AppBundle:Image_Comments','ii')
		->leftjoin('AppBundle:Users','u','WITH','u.id = ii.user_id')
		->orderBy('ii.add_date', 'ASC')
		->where('ii.image_id = :image_id')
		->setParameter(":image_id",$image_id)->getQuery()->getResult();
	...
	public function getimage_subcomments($image_comment_id_array)
	{
		...
		->createQueryBuilder()->select('ii.id,ii.user_id,ii.comment_id,ii.add_date,ii.text',
			'u.name,u.surname,u.profile_img')
		->from('AppBundle:Image_Subcomments','ii')
		->join('AppBundle:Users','u','WITH','u.id =ii.user_id')
		->orderBy('ii.add_date', 'ASC')
		->where('ii.comment_id in(:image_comment_id_array)')
		->setParameter(":image_comment_id_array",$image_comment_id_array)
		->getQuery()->getResult();
		
		return $subcomments;
	}
	
	public function getgroup_comments($group_id)
	{
		$comments = $this->entityManager
		->createQueryBuilder()
		->select('ii.id,ii.user_id,ii.add_date,ii.text','u.name,u.surname,u.profile_img')
		->from('AppBundle:Group_Comments','ii')
		->leftjoin('AppBundle:Users','u','WITH','u.id = ii.user_id')
		->orderBy('ii.add_date', 'ASC')
		->where('ii.group_id = :group_id')
		->setParameter(":group_id",$group_id)->getQuery()->getResult();
		
		return $comments;
	}
	
	public function getgroup_subcomments($group_comment_id_array)
	{
		$subcomments = $this->entityManager
		->createQueryBuilder()->select('ii.id,ii.user_id,ii.comment_id,ii.add_date,
			ii.text','u.name,u.surname,u.profile_img')
		->from('AppBundle:Group_Subcomments','ii')
		->join('AppBundle:Users','u','WITH','u.id =ii.user_id')
		->orderBy('ii.add_date', 'ASC')
		->where('ii.comment_id in(:group_comment_id_array)')
		->setParameter(":group_comment_id_array",$group_comment_id_array)
		->getQuery()->getResult();
	...
	public function add_image_commment($type,$image_comment,$image_id,$comment_id)
	{
		...
		if($type == 0){
			...
			$subcomment->setcomment_id($comment_id);
	...
	public function add_group_commment($type,$group_comment,$group_id,$comment_id)
	{
		$this->get_user_data();
		
		if($type == 1){
			$comment = new Group_Comments();
			$comment->setuser_id($this->user_data_array[0]);
			$comment->setgroup_id($group_id);
			$comment->setadd_date(new \DateTime(date("Y-m-d H:i")));
			$comment->settext($group_comment);
		...
		if($type == 0){
			$subcomment = new Group_Subcomments();
			$subcomment->setuser_id($this->user_data_array[0]);
			$subcomment->setcomment_id($comment_id);
			$subcomment->setadd_date(new \DateTime(date("Y-m-d H:i")));
			$subcomment->settext($group_comment);
	...
	public function get_groups($category,$search_input)
	{
		if($category == ''){
			...
			->orderBy('g.title', 'ASC')
			->where('g.title LIKE :title and g.type = 0')
			->setParameter(":title", '%'.$search_input.'%');
			...
			->getRepository(Groups::class)->createQueryBuilder('g')
			->orderBy('g.title', 'ASC')
			->where('g.title LIKE :title and g.type = 0 and g.category = :category')
			->setParameter(":title", '%'.$search_input.'%')
		...
		if($group != NULL){
			if($group->gettype() == 1){
				if($group->getowner_id() != $this->user_data_array[0]){
					...
					->getRepository(Group_Members::class)->findOneBy([
						'user_id' => $this->user_data_array[0],'group_id' => $id
					]);
	...
	public function get_group_users($id)
	{
		...
		->createQueryBuilder()->select('g.user_id ,u.name,u.surname,u.city,u.birthdate,
			u.profile_img')
		->from('AppBundle:Group_Members','g')
		->join('AppBundle:Users','u','WITH','u.id =g.user_id')
		->orderBy('CONCAT(u.name,  \' \',u.surname)', 'ASC')
		->where('g.group_id = :id')
	...
	public function get_user_groups($user_id = '')
	{
		...
		if($user_id == $this->user_data_array[0])
			...
			->getRepository(Groups::class)->createQueryBuilder('g')
			->orderBy('g.title', 'ASC')
			->where('g.id in  (:user_groups_id)')
			->setParameter(":user_groups_id",$group_id_array)
			->getQuery()->getResult();
		else
			$user_groups = $this->entityManager
			->getRepository(Groups::class)->createQueryBuilder('g')
			->orderBy('g.title', 'ASC')
			->where('g.id in  (:user_groups_id) and g.type = 0')
			->setParameter(":user_groups_id",$group_id_array)
			->getQuery()->getResult();
	...
	public function unjoin_group($group_id)
	{
		...
		$group_member = $this->entityManager
		->getRepository(Group_Members::class)->findOneBy([
			'group_id' => $group_id,'user_id' => $this->user_data_array[0]
		]);
